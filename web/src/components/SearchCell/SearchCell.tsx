...
export const Success = ({ search, query }: CellSuccessProps<FindSearchQuery>) => {
  return (
    <SearchLayout
      users={
        <>
          {search.users.length > 0 ? search.users.map((user) => (
      ...
      posts={
        <>
          {search.posts.length > 0 ? search.posts.map((post) => (
